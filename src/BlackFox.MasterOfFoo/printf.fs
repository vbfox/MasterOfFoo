...
module internal PrintfImpl =
    ...
    type ObjectPrinter =
        ...
        static member GenericToStringCore(v: 'T, opts: FormatOptions, bindingFlags): string =
            let vty =
                match box v with
                | null -> typeof<'T>
                | _ -> v.GetType()
        ...
        let buildStep (spec: FormatSpecifier) (argTys: Type[]) prefix =
            if spec.TypeChar = 'a' then
                ...
                | _ ->
                    ...
                    if isTwoStar then
                        let convFunc = conv.FuncObj :?> (obj -> int -> int -> PrintableElement)
                        StepStar2 (prefix, convFunc)
                    else
                        let convFunc = conv.FuncObj :?> (obj -> int -> PrintableElement)
                ...
                | _ ->
                    ...
                    let conv = getValueConverter argTy spec
                    let convFunc = conv.FuncObj :?> (obj -> PrintableElement)
